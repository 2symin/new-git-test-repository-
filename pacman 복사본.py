...
class Pacman(Entity):
    def __init__(self, level, start_pos):
        ...
        try:
            base_path = 'res'
            #path1 = os.path.join(base_path, 'pacman_1.png')
            #path2 = os.path.join(base_path, 'pacman_2.png')
            path1 = './res/pacman_1.png'
            path2 = './res/pacman_2.png'
...
class Ghost(Entity):
    ...
    def update(self, pacman, blinky=None, game_controller=None):
        if self.state == GHOST_STATE_EATEN and self.tile_pos == self.start_pos:
            ...
            game_controller.add_to_exit_queue(self)
    ...
    def reset(self): super().reset(); self.state, self.speed, self.calculated_path, self.is_immune = GHOST_STATE_IN_HOUSE, GHOST_BASE_SPEED, None, False
...
class Blinky(Ghost):
    def __init__(self, level, start_pos):
        ...
        try:
            base_path = 'res'
            #blinky_img = pygame.image.load(os.path.join(base_path, 'blinky.png')).convert_alpha()
            blinky_img = pygame.image.load('./res/blinky.png').convert_alpha()
...
class Pinky(Ghost):
    def __init__(self, level, start_pos):
        ...
        try:
            base_path = 'res'
            #pinky_img = pygame.image.load(os.path.join(base_path, 'pinky.png')).convert_alpha()
            pinky_img = pygame.image.load('./res/pinky.png').convert_alpha()
...
class Inky(Ghost):
    def __init__(self, level, start_pos):
        ...
        try:
            base_path = 'res'
            #inky_img = pygame.image.load(os.path.join(base_path, 'inky.png')).convert_alpha()
            inky_img = pygame.image.load('./res/inky.png').convert_alpha()
...
class Clyde(Ghost):
    def __init__(self, level, start_pos):
        ...
        try:
            base_path = 'res'
            #clyde_img = pygame.image.load(os.path.join(base_path, 'clyde.png')).convert_alpha()
            clyde_img = pygame.image.load('./res/clyde.png').convert_alpha()
...
class GameController:
    ...
    def start_new_game(self):
        ...
        try:
            base_path = 'res'
            #frightened_img = pygame.image.load(os.path.join(base_path, 'frightened.png')).convert_alpha()
            #frightened_flash_img = pygame.image.load(os.path.join(base_path, 'frightened_flash.png')).convert_alpha()
            #eyes_img = pygame.image.load(os.path.join(base_path, 'eyes.png')).convert_alpha()
            frightened_img = pygame.image.load('./res/frightened.png').convert_alpha()
            frightened_flash_img = pygame.image.load('./res/frightened_flash.png').convert_alpha()
            eyes_img = pygame.image.load('./res/eyes.png').convert_alpha()
        ...
        self.exit_queue = list(self.ghosts)
    ...
    def add_to_exit_queue(self, ghost): 
        if ghost not in self.exit_queue: self.exit_queue.append(ghost)
    ...
    def update_ghosts(self):
        if self.exit_queue:
            token_holder = self.exit_queue[0]
            if token_holder.state == GHOST_STATE_IN_HOUSE: token_holder.state = GHOST_STATE_EXITING
            if token_holder.tile_pos == self.level.ghost_house_exit:
                exiting_ghost = self.exit_queue.pop(0)
                if self.frightened_timer > 0 and not exiting_ghost.is_immune:
                    exiting_ghost.state, exiting_ghost.speed = GHOST_STATE_FRIGHTENED, GHOST_FRIGHTENED_SPEED
                else: exiting_ghost.state = self.ghost_mode
        ...
        for ghost in self.ghosts:
            if ghost.state not in [GHOST_STATE_FRIGHTENED, GHOST_STATE_EATEN, GHOST_STATE_IN_HOUSE, GHOST_STATE_EXITING]: ghost.state = self.ghost_mode
